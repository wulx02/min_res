# Licensing

This repository is a combined distribution under the SPDX expression
`MIT AND Apache-2.0`:

- Original code and generated data in this repository are licensed under the
  [MIT License](LICENSE).
- Portions translated or adapted from SSeqCpp or SpectralSequences/sseq are
  distributed under the [Apache License 2.0](LICENSE-APACHE).

Recipients of the combined distribution must comply with both licenses. The
origin and licensing of the affected source code are described in
[PROVENANCE.md](PROVENANCE.md) and
[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).

The GPL-2.0-licensed cnassau/steenrod project is cited only as a related
implementation of Christian Nassau's published algorithm. No source code from
cnassau/steenrod is copied, translated, linked, or distributed here.
